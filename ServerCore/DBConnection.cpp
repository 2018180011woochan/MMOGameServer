...
bool DBConnection::Connect(SQLHENV henv, const WCHAR* connectionString)
{
	...
	if (ret == SQL_ERROR || ret == SQL_SUCCESS_WITH_INFO)
	{
		...
		{
			std::wcout << L"=== ODBC Error Information ===" << std::endl;
			std::wcout << L"Return Code: " << ret << std::endl;
			std::wcout << L"SQL State: " << sqlState << std::endl;
			std::wcout << L"Native Error: " << nativeError << std::endl;
			std::wcout << L"Error Message: " << messageText << std::endl;
...
bool DBConnection::BindParam(int32 paramIndex, int8* value, SQLLEN* index)
{
	...
	return BindParam(paramIndex, SQL_C_TINYINT, SQL_SMALLINT, size32(int8), value, index);
}
...
{
	SQLLEN size = static_cast<SQLULEN>( ( ::wcslen(str) + 1 ) * 2);
	*index = SQL_NTSL;
...
bool DBConnection::BindParam(int32 paramIndex, const BYTE* bin, int32 size, SQLLEN* index)
{
	...
	if (size > BINARY_MAX)
		...
		return BindParam(paramIndex, SQL_C_BINARY, SQL_C_BINARY, size, (BYTE*)bin, index);
